...
async def authenticate(request: LoginRequest, db: AsyncSession = Depends(get_db)):
    """
    ...
    if not user:
        raise HTTPException(
            ...
            detail={"error": "Неверный логин или пароль"},
        )
    ...
    return LoginResponse(name=user.name, access_token=access_token)
